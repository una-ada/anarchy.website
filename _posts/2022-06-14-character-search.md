...
scraping to offer greater read functionality than the current [Offical MAL
...
the devloper's Client ID in the `X-MAL-CLIENT-ID` header! Such a system is
...
unofficial one. If you're building and application that will heavily integrate
...
seasonal rankings, and the authenticated used. It can also update user lists and
...
Let's just put MyAnimeList aside and look at some other database's APIs; after
...
`voiceActors` fields being `null` in GraphiQL. Regardless, looking possible is
...
The question, then, is what will it take to switch my current backend to
something more conducive to GraphQL queries. Since I'm alreay going to be making
GraphQL queries to AniList's API, I might as well use GraphQL for my end to end,
